...
class ScoreService {
  ...
  async calculateTaskScore(taskId: string, completedAt: Date = new Date()): Promise<number> {
    ...
    const finalScore =
      this.BASE_SCORE * diffMultiplier * prioMultiplier * (1 + bonus - penalty) * qualMultiplier;
  ...
  async getUserScore(userId: string, periodo?: string): Promise<number> {
  ...
  async getTeamScore(teamId: string, periodo?: string): Promise<number> {
